// Вебхуки платёжных мерчантов — единственные роуты admin API, доступные без
// авторизации (регистрируются вне группы /api, см. router.go), поскольку
// вызывающая сторона — сервер мерчанта, а не залогиненный админ.
package handlers
