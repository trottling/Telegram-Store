...
internal/cache/redis/         the ONE Redis-backed struct implementing domain/cache's per-entity interfaces,
                               ...
                               state, admin_session:<hash>/admin_exchange_attempts:<ip> for the web panel's login flow
...
admin_backend/middleware/      auth.go (bearer session token -> AdminAuthService.ValidateSession, attaches
                               *models.User to request context), cors.go (comma-separated allowed-origin list via
                               ...
                               against the real IP), metrics.go (Metrics — HTTP request count/duration, see the
                               observability-stack paragraph above for why it's registered before gin.Recovery()
                               and why its metric names can't be shared with payments_backend's twin)
...
payments_backend/handlers/     handler.go (the Handlers struct/constructor — two services plus the CrystalPay
                               ...
                               domain interface. CrystalPayWebhook/TinkoffWebhook increment
                               paymentsmetrics.WebhookSignatureInvalidTotal{merchant} right at their signature-
                               mismatch return — YooKassa has no counterpart since it never signs at all
payments_backend/router.go,    three POST routes (/api/webhooks/{crystalpay,yookassa,tinkoff}), middleware.Metrics
  server.go                    + gin.Recovery() + middleware.Detach, no Auth group, no CORS — http.Server
                               lifecycle (Start/Shutdown)
payments_backend/middleware/   detach.go (Detach) — a correctness reason, not tidiness: gin cancels the request
                               ctx the moment the client disconnects, and payment merchants cut the connection
                               on their own few-second
                               ...
                               row stays pending, the merchant's retry still credits). metrics.go (Metrics — HTTP
                               request count/duration, registered before gin.Recovery() for the same reason as
                               admin_backend's own copy, see the observability-stack paragraph above)
