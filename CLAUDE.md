...
The actual admin UI is a separate web panel: a Go+gin JSON API (`admin_backend`, its own binary/container — `cmd/admin_backend`), plus a React+Ant Design frontend (`admin_frontend/`, its own container). In `docker-compose.debug.yml` they talk cross-origin (frontend on `:3000`, API on `:8080`, hence `middleware.CORS`); behind `caddy` in prod they're the same origin — one domain, routed by path (`/` → panel, `/api` → API, see the Caddyfile section below) — so a browser there never even sends a CORS preflight. Auth has no persistent credential at all: `/admin` issues a 30-second one-time code, the login page exchanges it for a JWT session token (also backed by a Redis-held revocation record), and a fresh code is issued every time — nothing admin-related is ever stored in Postgres.
