...
- 🔄 **Проверка оплаты** — кнопка «Проверить оплату» под счётом на случай, если вебхук мерчанта задержался или не пришёл; подтверждённый платёж сразу меняет карточку на «Баланс пополнен»
...
- 📦 **CRUD каталога** — категории (дерево любой глубины) и товары создаются, редактируются и удаляются прямо в панели
